...
const createCommentValidation = [
    ...
    .isMongoId().withMessage("Invalid Id").bail({level : 'request'})
    .custom(async (value) => {
        let commentData = await CommentModel.findOne({_id : value, user_id: req.user.id});
        if (!commentData) {
    ...
    body('post_id')
        .optional({nullable: true})
        .isMongoId().withMessage("Invalid Post Id").bail()
...
const getCommentValidation = [
    param('post_id')
    .notEmpty().withMessage('post_id is required').bail()
    .isMongoId().withMessage("Invalid Post Id").bail()
    .custom(async (value, { req }) => {
